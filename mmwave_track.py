...
alarm_flag = -1
pwm.pwm_init()
tcpServPoint = threading.Thread(target=TCPPoint)
tcpServPoint.start()
#global xy
    ...
    while lostsync:
        ...
        if num1 == 7:
           ...
           # print()
            ...
            print('realframenumber is ',realframenumber,'\n')
    ...
    if lostsync == 0:
        ...
        if alarm_flag == -1:                              #未有报警targets
            for num2 in range(0,numtargets):             #轮询各个TARGETS
                ...
                if(V_mod>1 and A_mod>4):
                    print('V_mod is',V_mod)
                    print('A_mod is',A_mod)
                print('-------------------------------------------')
                if (A_mod > 0):  #初步判断 
                    target_id = []
                    ...
                    #print('ypfloat is ')
                    #print(ypfloat)
                    pwm.pwm_chan_track(xpfloat,ypfloat) #安防
            ...
            if numtargets == 0:
                ...
                for num3 in range(0,numtargets):  #获取报警target最新位置
                    if targets[num3][0:4] == target_id:
                        AX = convert(targets[num3][20:24])
                        AY = convert(targets[num3][24:28])
                        VX = convert(targets[num3][12:16])
                        VY = convert(targets[num3][16:20])
                        #print('AX is')
                        #print(AX)
                        #print('AY is')
                        #print(AY)
                        ...
                        #print('ypfloat is ')
                        #print(ypfloat)
                        pwm.pwm_chan_track(xpfloat,ypfloat)
                        #pwm.pwm_chan(xpfloat,ypfloat)
                        #time.sleep(0.5)
                        break
                    # else:
